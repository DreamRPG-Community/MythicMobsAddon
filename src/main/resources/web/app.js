(() => {
  ...
  const mcClasses = { color: 'mc-color-', bold: 'mc-bold', italic: 'mc-italic', underline: 'mc-underline', strike: 'mc-strike', obfuscated: 'mc-obfuscated' };
  const isMinecraftCode = code => /^[0-9a-fk-or]$/i.test(code);
  const isMinecraftColor = code => /^[0-9a-f]$/i.test(code);
  const minecraftStyle = color => ({ color, bold: false, italic: false, underline: false, strike: false, obfuscated: false });
  ...
  const selectedMaterial = () => {
    ...
    return entry || state.editorCatalog.materials.find(item => item.materialName === state.materialName) || { materialName: state.materialName, data: state.materialData, displayName: state.materialName };
  };
  ...
  const renderRows = items => {
    ...
    for (const item of items) {
      ...
      const tagEntries = (item.classification?.tagIds || []).map(id => tags.get(id) || { id, displayName: id, color: '#929394' });
  ...
  const commitImport = async () => {
    ...
    try {
      ...
      if ($('import-dialog').open && state.importResult?.status !== 'IMPORTED') {
        renderImportResult(state.importResult || { status: 'FAILED', message: '导入失败', fileCount: 0, candidates: [], conflicts: [], warnings: [], errors: [] });
      }
    }
  };
  ...
  $('tag-filter').onchange = () => { clearSelection(); state.page = 0; loadItems().catch(showError); };
  $('search').oninput = () => { clearSelection(); state.page = 0; loadItems().catch(showError); };
  $('previous').onclick = () => { state.page = Math.max(0, state.page - 1); loadItems().catch(showError); };
  $('next').onclick = () => { state.page += 1; loadItems().catch(showError); };
